---
title: "Researcher Training and Development Sessions for 2021 to mid-2024"
subtitle: "Enrolment and Attendance Statistics by College"
#author: "THIS IS NOT READY TO RUN--I'M STILL PLAYING WITH IT"
#date: "19/10/2023"
output: pdf_document
editor_options: 
...
# load data, add date, and select relevant variables

dfi_24 <- read_csv(here::here("data", "mid_2024_ienrol_data.csv"), lazy = FALSE) %>% 
  clean_names() %>% 
  mutate (year = "2024") %>% 
  relocate(year) %>% 
  select (c(year, course_name, event_num, date, first_name, last_name, fan, department, job_status, status)) 

dfi_23 <- read_csv(here::here("data", "2023_ienrol_data.csv"), lazy = FALSE) %>% 
  clean_names() %>% 
  mutate (year = "2023") %>% 
  relocate(year) %>%
  select (c(year, course_name, event_num, date, first_name, last_name, fan, department, job_status, status)) 

dfi_22 <- read_csv(here::here("data", "2022_ienrol_data.csv"), lazy = FALSE) %>% 
  clean_names() %>% 
  mutate (year = "2022") %>% 
  relocate(year) %>%
  select (c(year, course_name, event_num, date, first_name, last_name, fan, department, job_status, status)) 

dfi_21 <- read_csv(here::here("data", "2021_ienrol_data.csv"), lazy = FALSE) %>% 
  clean_names() %>% 
  mutate (year = "2021") %>% 
  relocate(year) %>%
  select (c(year, course_name, event_num, date, first_name, last_name, fan, department, job_status, status)) 
...
dfi_all <- list(dfi_21, dfi_22, dfi_23, dfi_24) %>% reduce(full_join)
...
dfi_all <- dfi_all %>% 
  mutate("date" = dmy(dfi_all$date)) %>% 
  mutate("month" = month(dfi_all$date)) %>% 
...
```{r delete sessions that did not run, eval=FALSE, include=FALSE}
...
# update July 2024. We used to run this after we cleaned the data (FYI), but it looks like the ienrol data
  # is no longer showing the cancelled sessions. We will have to check this again, but for now, there's no 
  # need to run it. 
...
# join all of these 'to be removed' tibbles together
dfi_23_rm <- list(dfi_23_a, dfi_23_b, dfi_23_c, dfi_23_d, dfi_23_e) %>% reduce(full_join)

#check the numbers for 2023
dfi_23 <- anti_join (dfi_23, dfi_23_rm)

# remove it from the full data
dfi <- anti_join (dfi, dfi_23_rm)

rm(dfi_23_a, dfi_23_b, dfi_23_c, dfi_23_d, dfi_23_e, dfi_rm)
...
n_courses <- dfi_all %>% 
  ...
  dplyr::count(year, course_name, event_num)
...
n_attendees <- dfi_all %>%
  filter(attendance_status == "Attended") %>% 
  dplyr::count(year, fan, course_name, event_num)

# attendees by year
year_attendees <- plyr::count(n_attendees, "year")
...
attended <- dfi_all %>% filter(attendance_status == "Attended")
not_attended <- dfi_all %>% filter(attendance_status == "Did not attend")


```


```{r enrolment and attendance data, eval=TRUE}

# summarise the data based on how many courses were held and how many folks enrolled in each of the sessions
...
summary <- n_courses %>% 
  pivot_wider(names_from = attendance_status, values_from = n) %>% 
  mutate ("month" = as.character(month)) %>%  # we need to do this so it's not included in the total
  adorn_totals("col") %>% 
  mutate ("attendance_rate" = round((`Attended`/`Total`)*100,2)) 


# now get the summary data by month so we group it that way in the graph

df <- dfi_all %>% 
    group_by(month = floor_date(date, unit = "month"), attendance_status) %>% 
   dplyr::count(year, month) %>% 
    pivot_wider(names_from = attendance_status, values_from = n) %>% 
    adorn_totals("col") %>% 
  mutate ("attendance_rate" = round((`Attended`/`Total`)*100,2)) %>% 
  mutate ("attendance_prop" = round((`Attended`/`Total`),2))

# now get the summary data by quarter so we group it that way in the graph

df_q <- dfi_all %>% 
  mutate("qtr" = paste0(substring(year(date), 3,4),"/Q", quarter(date))) %>%  #create the quarter labels
    group_by(qtr, attendance_status) %>% 
   dplyr::count(qtr) %>% 
    pivot_wider(names_from = attendance_status, values_from = n) %>% 
    adorn_totals("col") %>% 
  mutate ("attendance_rate" = round((`Attended`/`Total`)*100,2)) %>% 
...
# # Attendance status
# 
# attended <- dfi %>% filter(attendance_status == "Attended")
# not_attended <- dfi %>% filter(attendance_status == "Did not attend")
...
# this works to show the data grouped by year and month
...
# trying to add the attendance rate as a line

ggplot(df) +
  geom_bar(aes(x = month, y = Total), stat = "identity") +
  geom_line(aes(x = month, y = 100*attendance_rate), stat = "identity", color = "red", linewidth = 2) +
scale_x_date(NULL, date_labels = "%b %y", breaks = "month") + 
  scale_y_continuous(sec.axis=sec_axis(~.*0.01, name = "Percentage"))


# trying to add the attendance rate as a line [breadcrumb: the secondary axis is off; need to play with it more]

#https://www.geeksforgeeks.org/combine-bar-and-line-chart-in-ggplot2-in-r/
  
ggplot(df) +
  geom_bar(aes(x = month, y = Total), stat = "identity") +
  geom_line(aes(x = month, y = 100*attendance_prop), stat = "identity", color = "red", linewidth = 2) +
scale_x_date(NULL, date_labels = "%b %y", breaks = "month") + 
  scale_y_continuous(sec.axis=sec_axis(~.*0.01, name = "Percentage"))


# just plot attendance rates as a line
ggplot(df) +
 # geom_bar(aes(x = month, y = Total), stat = "identity") +
  geom_line(aes(x = month, y = attendance_rate), stat = "identity", color = "red", linewidth = 2) +
scale_x_date(NULL, date_labels = "%b %y", breaks = "month") +
  scale_y_continuous(name = "Attendance Rates", limits = c(0, 100))
...
# this works to show the data grouped by quarter


ggplot(df_q, aes(x = qtr, y = Total)) + 
  geom_bar(stat = "identity") + 
  xlab ("\nYear & Quarter") +
...
ggplot(df_q) +
  geom_bar(aes(x = qtr, y = Total), stat = "identity") +
  geom_line(aes(x = qtr, y = 100*attendance_prop), color = "red", linewidth = 2) +
  xlab ("\nYear & Quarter") +
  ylab ("Total registrations") + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 1)) +
  scale_y_continuous(sec.axis=sec_axis(~.*0.01, name = "Proportion"))


# we can't use the secondary axis because it's not scaled from the primary axis. I need to use 
  # use facet wrap, and use two different graphs [breadcrumb, I will need to make my data long first]

# https://ggplot2.tidyverse.org/reference/facet_wrap.html

ggplot(df_q) +
  geom_line(aes(x = qtr, y = Total, group = 1), stat = "identity", color = "blue", linewidth = 2) +
  geom_line(aes(x = qtr, y = attendance_rate, group = 1), stat = "identity", color = "red", linewidth = 2) +
  xlab ("\nYear & Quarter") +
  facet_wrap(~ symbol, nrow = 2, scales = "free")


# just plot attendance rates as a line [this works]
ggplot(df_q) +
 # geom_bar(aes(x = month, y = Total), stat = "identity") +
  geom_line(aes(x = qtr, y = attendance_rate, group = 1), stat = "identity", color = "red", linewidth = 2) +
  xlab ("\nYear & Quarter") +
...
```{r attendance data, eval=FALSE}
# I DON'T YET KNOW IF I NEED THIS DATA
...
df_grouped <- dfi_all %>%
  dplyr::count(portfolio,attendance_status) %>% 
    group_by(portfolio) %>% 
    mutate(att_pct = (n/sum(n)) *100) %>%
    mutate(enrol = sum(n))


# figure out how to plot the data by the dates
...
# plot it [this has enrollment & attendance rates across the months and years]
ggplot(summary, aes(
  x = (year, month),
  y = Total
)) +
