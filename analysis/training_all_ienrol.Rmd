...
dfi_2024 <- read_csv(here::here("data", "mid_2024_ienrol_data.csv"), lazy = FALSE) %>% 
  clean_names()

dfi_2023 <- read_csv(here::here("data", "2023_ienrol_data.csv"), lazy = FALSE) %>% 
  clean_names()

dfi_2022 <- read_csv(here::here("data", "2022_ienrol_data.csv"), lazy = FALSE) %>% 
  clean_names()

dfi_2021 <- read_csv(here::here("data", "2021_ienrol_data.csv"), lazy = FALSE) %>% 
  clean_names()
...
dfi_full <- dfi_full %>% 
  mutate("year" = lubridate::year(dfi_full$date)) %>% 
  relocate (year)

# select relevant variables 

dfi <- dfi_full %>%
  select (c(year, course_name, event_num, first_name, last_name, fan, department, job_status, status)) 
...
n_attendees <- dfi %>%
  filter(attendance_status == "Attended") %>% 
  count(year, fan, course_name, event_num)
