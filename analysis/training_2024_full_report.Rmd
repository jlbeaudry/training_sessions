...
df_sp <- read_xlsx(
  here::here("data",
             "2024_training_data_q3.xlsx"),
  sheet = '2024 Program',
  skip = 5, #start at the header row
  n_max = 109,  #last session TD - RN info session
  .name_repair = "unique",
  trim_ws = TRUE)

# load data from ienrol (this contains names of Flinders attendees & includes college/portfolio info)

df_ienrol <- read_csv(here::here("data", "2024_ienrol_data_q3.csv"), lazy = FALSE) %>% 
  clean_names()
...
As of October 28, 2024, the Training, Development and Communication team in Research
...
staff at Flinders University. We have 21 sessions scheduled for the remainder of the year.

From the Flinders community, `r nrow(n_attendees_ienrol)` people attended the research training and
development sessions thus far in 2024. Our actual attendance numbers for the year are larger (*n* = `r sum(df_sp$Attended)`)
because we allowed external researchers to join select sessions (e.g., Open
Research sessions, etc.). Of the `r nrow(n_attendees_ienrol)`
attendees from Flinders, `r n_unique_attendees_ienrol` were unique attendees, demonstrating
...
hands-on ethics workshop to `r max(n_courses$n)` in a 'Writing for The Conversation' session. 
We had only the one workshop with no attendees. The mean number of attendees at our sessions was `r round(mean(n_courses$n),0)` with a median of `r round(median(n_courses$n),0)`.
...
(including Library; Information and Digital Services; Chief of Staff Portfolio; and 
Governance, Legal and Risk; as well as people who are not
...
Only (`r round(df_attend %>% filter(job_status=="Student") %>% select(n)/sum(df_attend$n)*100 ,2)`%) were students;
...
```{r unique sessions by theme, include = TRUE, eval=TRUE}
...
unique_theme_sessions <- df_sp %>% 
    arrange(Session)
  mutate(Session = as.factor(Session)) %>% 
  unique()

    unique(df_sp$Session)
