...
n_attendees_hass <- df %>%
  filter(attendance_status == "Attended") %>% 
  count(fan, course_name, event_num, portfolio) %>% 
  filter(portfolio == "College of Humanities, Arts and Social Sciences")

n_unique_attendees_hass <- n_distinct(n_attendees_hass$fan)
...
ggplot(hass_grouped, aes(
  ...
  geom_text(aes(y = n,
            ...
            size = 4) +
  coord_flip() +
  labs (title = "Attendance by Course Name (2023)", 
        x = element_blank(),
        y = "Number of enrolments") +
  theme_classic() +
  theme(
