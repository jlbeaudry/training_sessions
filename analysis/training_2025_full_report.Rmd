...
df_sp <- read_xlsx(
  ...
  n_max = 129,  #last session was MRFF strategic planning
  .name_repair = "unique",
  trim_ws = TRUE)
...
n_courses_all_ienrol <- df_ienrol %>% 
#  filter(attendance_status == "Attended") %>% 
  count(course_name, event_num)

n_courses_ienrol <- df_ienrol %>% 
  filter(attendance_status == "Attended") %>% 
  count(course_name, event_num)

# In 2025, we started including Mary's visits to the Colleges (this is the first year she did that) 
  # because we are going to support this more in the future. She doesn't have attendance data 
  # for those visits, so we have not included them in iEnrol. As such, there is a disconnect between
  # the number of sessions captured in the spreadsheet vs. ienrol. We will report this in the report. 
...
```{r clean spreadsheet data & calculate number of courses}
...
# number of courses from spreadsheet rather than ienrol because Mary's sessions can't be captured in ienrol

n_courses <- nrow(df_sp)
...
Development and Support have offered `r n_courses` information
...
research training and development sessions in 2025. Our actual attendance numbers 
...
the inaugural NADOIC Indigenous Research Showcase, etc.). We also report information
sessions that were delivered individually to College forums (*n* = `r missing_ienrol`), 
but we note that the attendance numbers were estimated and the attendees could not 
be named or captured in the iEnrol system. 
...
The number of attendees at our sessions varied from `r min(df_sp$Attended)` in one session
(i.e., a hands-on workshop for using ResearchNow) to `r max(df_sp$Attended)` 
in our first session on Microsoft CoPilot and AI for research. The mean 
number of attendees at our sessions was `r round(mean(df_sp$Attended),0)` with a median of 
`r round(median(df_sp$Attended),0)`.
...
number of attendees across the Colleges and Portfolios. As would be expected for the 
largest College, the College of Medicineand Public Health had the most unique attendees.
...
## Overview of Research Training and Development Themes and Topics
...
The fact that this theme also had the largest number of attendees is evidence 
that the research community values these training and development opportunities.
...
Of the `r nrow(df_sp)` sessions that we offered in 2025, `r nrow(n_event_sp)` 
(`r round((nrow(n_event_sp))/nrow(df_sp)*100, 0)`%)
...
(*n* = `r (nrow(df_sp)-nrow(n_event_sp))`; `r round(((nrow(df_sp)-nrow(n_event_sp))/(nrow(df_sp))*100),0)`%) 
...
attended in person (*n* = `r sum(n_event_sp$Attended_in_person)`; 
...
kable(theme_overview,
      ...
      align = c('lcc'),
      caption = "Training and Development Summary for 2025",
      linesep = "") %>% #get rid of spacing every five lines
  kable_styling(latex_options = c("hold_position", "striped"))
